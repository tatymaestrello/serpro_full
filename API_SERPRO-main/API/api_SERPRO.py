...
def criar_nova_pasta(cliente):
    ...
    response = requests.post(url=url+"folders", json=dados_nova_pasta, auth=(Username,Passord))
...
def buscar_dados_pasta(cliente):
    ...
    response = requests.get(url=url+"folders/"+cliente, auth=(Username,Passord))
...
def importar_dashboard(nome_arq_json,cliente,folderId,folderUid,path,flag_menuportal,lista_itens_menu):
    erro_html_data = ""
    ...
    if os.path.isfile(path_modelo+nome_arq_json):
        ...
        str_json_dashboard = str_aux.replace("MODELO",str(cliente).upper()).replace("modelo",str(cliente).lower())
        ...
        if flag_menuportal == True:
            ...
            if html_data != 'null':
                json_dashboard.get("panels")[0]["html_data"] = json.loads(modificar_html_menuportal(html_data,lista_itens_menu).encode('utf-8'))
            else:
                erro_html_data = " Erro: 'html_data' do 'meuportalmodelo' está em branco."
        ...
        response = requests.post(url=url+"dashboards/import", json=dados_dashboard, auth=(Username,Passord))
        ...
        if not(json.dumps(resp_novo_dashboard.get("title")).find(cliente) >= 0 or json.dumps(resp_novo_dashboard.get("uid")).find(cliente) >= 0): 
            return "Erro: Importação dashboard '"+nome_arq_json+"': "+json.dumps(resp_novo_dashboard) + erro_html_data
        return "Ok: Dashboard '"+nome_arq_json+"' importado corretamente!" + erro_html_data
...
def criar_cliente():
    ...
    if resp_pasta_grafana.get("title") == cliente: # pasta criada corretamente ou já existente
                    ...
                    if type(lista_itens_sec) is dict:
                        ...
                        if type(lista_itens_sec) is list and lista_itens_sec != []:
                            ...
                            if type(lista_itens_sec[1]) is str:
                                ...
                                lista_itens_ter = json.loads(json.dumps(lista_itens_sec[1]))
                                # terceiro nível
        ...
        for nome_dash in lista_nomes_dashboards_final:    
            lista_msg_resposta.append(importar_dashboard(nome_dash,cliente,folderId,folderUid,path,False,[]))

        # importando o menu 'menuportalmodelo'. Sempre ele é importado     
        lista_msg_resposta.append(importar_dashboard("menuportalmodelo",cliente,folderId,folderUid,path,True,lista_codigos_menu_final))

        return lista_msg_resposta
    
    if criar_pasta == 'S':
        return "Erro: Criação da pasta '"+cliente+"': "+json.dumps(resp_pasta_grafana)
    else:
